...
import { userLogin,recoverPassword} from '../../store/slice/userSlice';
...
useEffect(()=>{
    ...
    if(user !== null && user.statusCode === 200 && user.data.role === 'ADMIN'){
        dispatch(toggleOverlay(false)); 
        navigate('/'); 
    }

    if(user !== null && user.statusCode === 200 && user.data.role === 'USER'){
        setErr('Tài khoản không có quyền')
        dispatch(toggleOverlay(false)); 
    }
